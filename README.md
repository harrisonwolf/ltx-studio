...
- **Blind A/B**: queue two variants of one config, rate them blind, reveal after. Until REVEAL, the queue, archive, INSPECT and TIMING views hide the varied value and anything it changes (for a LENGTH pair: single vs chained, shot and frame counts; for a BACKEND pair: steps, fps and resolution). The LIVE tab's progress counters and the raw terminal log stay real, so a running render can still be followed, and timing can't be blinded: variants that render differently (e.g. EVOLVE redirects every seam, HOLD every third) take visibly different time. Rate from the ARCHIVE, not while watching them render. Blinds and ratings are logged to `runs/pair_blinds.jsonl` / `runs/pair_ratings.jsonl`.
...
- **Style presets**: named bundles of anchor words (`Cinematic`, `Golden Hour`, `Noir`, …) that append to the ANCHORS field. They stack, and you can add your own in `runs/style_presets.json`. Anchors are folded into the prompt on every run type.
